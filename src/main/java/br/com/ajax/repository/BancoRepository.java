...
public class BancoRepository implements Serializable {
	...
	public Banco buscarPorAgencia(String agencia) {
		try {
			return manager.createQuery("from Banco where upper(agencia) = :agencia", Banco.class)
					.setParameter("agencia", agencia).getSingleResult();
		} catch (NoResultException e) {
			return null;
		}
	}
